...
public interface EspacioRepository extends JpaRepository<Espacio, Integer> {
    ...
    // Consulta para buscar espacios disponibles seg&ntilde;n un filtro
    ...
    // Obtener todos los espacios ordenados por n&ntilde;mero ascendente
    ...
    // Buscar espacios por n&ntilde;mero
    List<Espacio> findByNumero(String numero);

    // Buscar espacios por n&ntilde;mero, ignorando may&ntilde;sculas y min&ntilde;sculas
    List<Espacio> findByNumeroIgnoreCase(String numero);

    // Buscar espacios que contengan el n&ntilde;mero como parte del string
    @Query("SELECT e FROM Espacio e WHERE e.numero LIKE %?1%")
    List<Espacio> listaPorNumeroLike(String filtro);

    // Buscar espacios por n&ntilde;mero exacto
    @Query("SELECT e FROM Espacio e WHERE e.numero = ?1")
    List<Espacio> listaPorNumeroIgualRegistra(String numero);

    // Buscar espacios por n&ntilde;mero, excluyendo el espacio actual
